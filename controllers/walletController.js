...
exports.WalletNotify = (req, res, next) => {
    console.log(colors.bgWhite.black('WalletNotify'));
    ...
    Wallet.findOne({ name: req.params.walletname })
        ...
        .then(wallet => {
            ...
            requestController.RpcRequest({ chain: "test", wallet: wallet.name }, dataString).then((rpc_res) => {
                    console.log(colors.bgRed('WalletNotift Transaction'));
                    console.log(colors.cyan(rpc_res));
                    console.log(colors.cyan(rpc_res.result.details));

                    var details = rpc_res.result.details.filter(element => element.category === 'receive');
                    if (details.length > 0) {
                        console.log(colors.gray(details.length + ' request sending...'));
